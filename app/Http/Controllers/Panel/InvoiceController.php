...
class InvoiceController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            // ---- Sorting configurable con lista blanca y fallback seguro
            $allowed = ['issue_date', 'created_at', 'id', 'amount', 'rate'];
            $default = Schema::hasColumn('invoices', 'issue_date')
                ? 'issue_date'
                : (Schema::hasColumn('invoices', 'created_at') ? 'created_at' : 'id');

            $sortBy  = $request->input('sort_by', $default);
            $sortDir = strtolower($request->input('sort_dir', 'desc')) === 'asc' ? 'asc' : 'desc';

            if (!in_array($sortBy, $allowed, true) || !Schema::hasColumn('invoices', $sortBy)) {
                $sortBy = $default;
            }
            ...
            return InvoiceResource::collection($invoices)
                ->additional(['total' => $invoices->total()]);
        } catch (AuthorizationException $e) {
            return response()->json(['message' => 'No tienes permiso para ver las facturas.'], 403);
        } catch (Throwable $e) {
    ...
    public function indexfilter()
    {
        try {
            ...
            $primarySort = Schema::hasColumn('invoices', 'issue_date') ? 'issue_date'
                : (Schema::hasColumn('invoices', 'created_at') ? 'created_at' : 'id');
