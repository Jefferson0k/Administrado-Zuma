<template>
  <DataTable ref="dt" :value="investments" dataKey="id" v-model:selection="selectedInvestments" :paginator="true"
    :rows="rowsPerPage" :totalRecords="totalRecords" :first="(currentPage - 1) * rowsPerPage" :loading="loading"
    :rowsPerPageOptions="[5, 10, 20, 50]"
    paginatorTemplate="FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink CurrentPageReport RowsPerPageDropdown"
    currentPageReportTemplate="Mostrando {first} a {last} de {totalRecords} inversiones" @page="onPage" scrollable
    ...
    <template #header>
      <div class="flex flex-wrap gap-3 items-center justify-between mb-3">
        ...
        <div class="flex flex-wrap gap-2">
          ...
          <MultiSelect v-model="selectedFilters" :options="filterOptions" optionLabel="label" optionValue="value"
            placeholder="Selecciona filtros..." :maxSelectedLabels="3" class="w-full md:w-20rem" />
        </div>
        ...
        <div class="flex flex-wrap gap-3 items-end">
          <div class="flex flex-column gap-2">
            <IconField>
              ...
              <InputText v-model="filters.razon_social" @input="onFilterSearch" placeholder="Buscar por razón social..."
                class="w-full md:w-20rem" />
            </IconField>
          </div>

          <!-- Filtro por Moneda -->
          <div v-if="selectedFilters.includes('moneda')" class="flex flex-column gap-2">
            <Select v-model="filters.currency" :options="currencyOptions" optionLabel="label" optionValue="value"
              placeholder="Selecciona moneda" @change="onFilterSearch" class="w-full md:w-12rem" />
          </div>

          <!-- Filtro por Estado -->
          <div v-if="selectedFilters.includes('estado')" class="flex flex-column gap-2">
            <MultiSelect v-model="filters.status" :options="statusOptions" optionLabel="label" optionValue="value"
              placeholder="Selecciona estados" @change="onFilterSearch" :maxSelectedLabels="2"
    ...
    <Column field="return" header="Retorno" sortable style="min-width: 8rem">
      <template #body="{ data }">
        {{ data.currency }} {{ formatCurrency(data.return, data.currency) }}
      </template>
    </Column>
...
const hasActiveFilters = computed(() => {
  return filters.value.razon_social ||
