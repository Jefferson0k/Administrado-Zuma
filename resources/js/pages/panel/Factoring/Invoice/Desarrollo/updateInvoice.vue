<template>
  <Dialog v-model:visible="visible" :style="{ width: '600px' }" header="Actualizar factura" :modal="true" @hide="handleClose">
    ...
    <template #footer>
      <div class="flex justify-between items-center w-full">
        ...
        <div class="flex gap-2">
          ...
          <Button label="Actualizar" icon="pi pi-check" severity="contrast" :loading="loading" @click="actualizarFactura" />
        </div>
      </div>
    </template>
  </Dialog>
...
watch([() => props.modelValue, () => props.facturaId], ([isVisible, facturaId]) => {
  if (isVisible && facturaId) {
    cargarDatosFactura();
  }
});
...
async function cargarDatosFactura() {
  ...
  try {
    ...
    factura.value = {
      ...
      amount: parseFloat(data.montoFactura) || null,
      financed_amount_by_garantia: parseFloat(data.montoAsumidoZuma) || null,
      rate: parseFloat(data.tasa) || null,
      estimated_pay_date: data.fechaPago ? parseDateFromString(data.fechaPago) : null,
      loan_number: data.loan_number || '',
      invoice_number: data.invoice_number || '',
      RUC_client: data.RUC_client || ''
    };

    // Si hay company_id, buscar y cargar la empresa
    if (data.company_id && data.razonSocial) {
      empresas.value = [{
        label: `${data.razonSocial}`,
        sublabel: `Empresa cargada`,
...
async function actualizarFactura() {
  ...
  try {
    ...
    // Llamada a la API
    const response = await axios.put(`/invoices/${props.facturaId}`, formData, {
      headers: {
    ...
    if (error.response?.status === 422) {
      ...
      console.error('Factura no encontrada:', error.response.data.message);
      toast.add({
        severity: 'error',
        summary: 'Error',
