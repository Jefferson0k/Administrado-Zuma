...
// Exponer los filtros para que el componente padre pueda acceder a ellos
...
async function exportToExcel() {
    try {
        const params = Object.fromEntries(
            Object.entries(filters.value).filter(([key, value]) => value !== null && value !== '')
        );

        // Crear una URL con los parámetros de filtros
        ...
        toast.add({
            severity: 'success',
            summary: 'Éxito',
            detail: 'Exportación iniciada correctamente',
            life: 3000
        });
    } catch (error) {
        ...
        toast.add({
            severity: 'error',
            summary: 'Error',
            detail: 'Error al exportar las facturas',
            life: 3000
        });
    }
}
...
function clearUnselectedFilters() {
    if (!showCurrencyField.value) {
        filters.value.currency = null;
    }
    if (!showAmountFields.value) {
        filters.value.min_amount = null;
        filters.value.max_amount = null;
    }
    if (!showRateFields.value) {
        filters.value.min_rate = null;
        filters.value.max_rate = null;
    }
...
function onFacturaDialogCancelled() {
    selectedFacturaId.value = null;
    showFacturaDialog.value = false;
}

function onDeleteConfirmed(response) {
    console.log('Factura eliminada:', response);
    loadData();
    showDeleteDialog.value = false;
    selectedFacturaId.value = null;
    selectedFacturaData.value = null;
}
...
function onUpdateConfirmed(response) {
...
<template>
    <div class="space-y-4">
                ...
                <div class="flex flex-wrap gap-2 items-center justify-between">
                    ...
                    <div class="flex flex-wrap gap-2">
                        <MultiSelect 
                            ...
                            optionLabel="label" 
                            optionValue="value" 
                            placeholder="Seleccionar campos de filtro"
                            class="w-15"
                            display="chip"
                        />
                        <div v-if="showSearchField" class="lg:col-span-2">
                            <IconField>
                                ...
                                <InputText 
                                    ...
                                    placeholder="Buscar por razón social, código..." 
                                    class="w-15"
                                />
                            </IconField>
                        </div>
        ...
        <showFacturas
            v-model="showFacturaDialog"
            :factura-id="selectedFacturaId"
            @cancelled="onFacturaDialogCancelled"
        />

        <updateInvoice
            v-model="showUpdateDialog"
            :factura-id="selectedFacturaId"
            @updated="onUpdateConfirmed"
