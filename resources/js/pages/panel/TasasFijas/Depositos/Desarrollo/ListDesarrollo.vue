    ...
    confirm.require({
        ...
        accept: async () => {
            try {
                ...
                await axios.post(`/movements/${id}/aceptar-tasas-fijas`);
...
const rechazar = async () => {
    ...
    confirm.require({
        ...
        accept: async () => {
            try {
                const id = selectedRow.value.id;
                await axios.post(`/movements/${id}/rechazar-tasas-fijas`);
...
const loadData = async (page = 1) => {
    loading.value = true;
    try {
        const response = await axios.get(`/movements/tasas-fijas?page=${page}`);
    ...
    <DataTable ref="dt" v-model:selection="selectedProducts" :value="products" dataKey="id" :paginator="true" :rows="10"
        ...
        <template #header>
            <div class="flex flex-wrap gap-2 items-center justify-between">
                <h4 class="m-0">Depósitos - Tasas Fijas</h4>
                <IconField>
                    <InputIcon>
